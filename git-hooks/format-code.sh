...
CONFIG_PATH="$(git -C "$(dirname "${BASH_SOURCE[0]}")" rev-parse --show-toplevel)/git-hooks/linters_configs"
...
LINTER_IGNORE+=([prettier]="${CONFIG_PATH}/.prettierignore")
LINTER_TYPES+=([prettier]="json;markdown;yaml")
function do_prettier() {
    prettier --config "${LINTER_CONFIG[prettier]}" --ignore-path "${LINTER_IGNORE[prettier]}" --write "$@"
}
